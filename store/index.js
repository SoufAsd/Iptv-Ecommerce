...
export const mutations = {
  ...
  SHOW_COMMAND(state, data) {
    if(data){
        state.cart = [];
        // console.log(data.commands,state.cart);
        // // state.cart.push(data.commands)
        for (const command of data.commands) {
            state.cart.push({
                ...command,
                cartQuantity: command.quantity,
                image: `/storage/images/${command.image}`,
                total: command.price,
              });
        }
...
export const actions = {
  async registerUser({ commit }, payload) {
    const ip = await this.$axios
      .$post("api/clientregister", payload.prod, payload.requestOptions)
      .then((data) => {
  ...
  async loginUser({ commit }, payload) {
    const ip = await this.$axios
      .$post("api/loginregister", payload.prod, payload.requestOptions)
      .then((data) => {
        commit("LOGIN", data);
      })
      .catch((error) => {
  ...
  async updatePanier({ commit }, payload) {
    try {
        state.token = this.$auth.strategy.token.get().split("Bearer ")[1];
        const data_req = {
          token: state.token,
        };
        const ip = await this.$axios.$post("api/get_unpaied_commande", data_req).then((datas)=>{
            commit("SHOW_COMMAND", datas);
        })
      } catch (error) {}
  },
